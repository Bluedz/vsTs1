...
import { SStockFormComponent } from './sstockForm.component'
import { SStockSearchComponent } from './sstockSearch.component'
@Component({
  templateUrl: 'sstock-list.html',
  //styleUrls: ['../../../assets/css/default.scss'],
})
export class SStockComponent {
  public stockList = [];
  public pageInfo;
    ...
    if (maDesc != '' && maDesc != undefined) {
      if (maDesc.indexOf('@') > -1) {
        ...
        for (var attr of attrs) {
          filter += " and (STO_DESCRIPTION like '%" + attr + "%' ) ";
        }
      } else {
        filter += " and (STO_DESCRIPTION like '%" + maDesc + "%' ) ";
      }
    }

    var _param = {
      "bizObj": "CSC_STORAGE_SAIC",
      "service": "selectMore",
      "fields": "*",
      "filter": filter,
  ...
  itemClick (mat) {
    this.navCtrl.push(SStockFormComponent, mat);
  }
  ...
  adSearch(filters) {
      this.navCtrl.push(SStockSearchComponent, filters);
  }
}
